...
### Changed
- **CI: authenticate before push**: Added `git remote set-url` with `GITHUB_TOKEN` in the changelog update workflow so the automated commit can be pushed back to the repository.

---

### 変更
- **CI: プッシュ前に認証を設定**: チェンジログ自動更新ワークフローで `git remote set-url` に `GITHUB_TOKEN` を使用するよう変更し、自動コミットをリポジトリへプッシュできるようにした。
