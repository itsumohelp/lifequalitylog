  ...
  try {
    ...
    if (exitingRecord) {
      return Response.json(
        { messge: "already exists", status: 400 },
        { status: 400 },
      );
    }
...
export async function PUT(
  ...
  try {
    ...
    if (!exitingRecord) {
      return Response.json(
        { messge: "target record do not exist", status: 404 },
        { status: 404 },
      );
    }
    ...
    for (const record of recordList) {
      const workMuinutes =
        record.ended && record.createdAt
          ? Math.floor(
              (record.ended.getTime() - record.createdAt.getTime()) / 60000,
            )
          : 0;
    ...
    return Response.json(
      { messge: "success", cost: totalMinutes },
      { status: 200 },
    );
  } catch (error) {
    console.error("Error creating come:", error);
