...
export default function CapacitorLoginButton({
  agreed,
  callbackUrl,
}: {
  agreed: boolean;
  ...
  const checkPoll = async (pollId: string) => {
    try {
      const res = await fetch(
        `https://crun.click/api/auth/ios-poll?pollId=${pollId}`,
      );
      const data = await res.json();
      if (data.token) {
        clearInterval(pollRef.current!);
        await fetch(
          `https://crun.click/api/auth/ios-session?token=${encodeURIComponent(data.token)}`,
          {
            credentials: "include",
          },
        );
  ...
  const handleAppleLogin = () => {
    (window as any).__appleAuthCompleted = async ({
      identityToken,
      firstName,
      lastName,
    }: {
      ...
      try {
        ...
        if (token) {
          await fetch(
            `https://crun.click/api/auth/ios-session?token=${encodeURIComponent(token)}`,
            {
              credentials: "include",
            },
          );
