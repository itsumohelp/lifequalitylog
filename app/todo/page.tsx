...
export default function TodoTimeTracker() {
  ...
  const pushTimer = (todoId: string, isActive: boolean) => {
    if (isActive) {
      ...
      timerIdRef.current = window.setInterval(() => {
        setTodos((todos) =>
          todos.map((todo) =>
            todo.id === todoId ? { ...todo, cost: todo.cost + 1 } : todo,
          ),
        );
      }, 60000); // 1分ごと
    } else {
  ...
  const toggleTimer = async (todoId: string, currentStatus: number) => {
    ...
    if (resCost === undefined)
      resCost = todos.find((todo) => todo.id === todoId)?.cost ?? 0;
    setTodos((todos) =>
      todos.map((todo) =>
        todo.id === todoId
          ? {
              ...todo,
              cost: resCost,
              record: { id: todo.record.id, status: currentStatus },
            }
          : todo,
      ),
    );
              ...
              >
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <span className={`text-gray-700}`}>
                    <span className={`text-gray-500`}>{todo.name}</span>
                    {todo.title}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-4">
                  {/* 時間表示 */}
                  <div
                    className={`font-mono text-xl w-28 text-right text-gray-600}`}
                  >
                    {todo.cost} 分
                  </div>
